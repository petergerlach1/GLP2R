...
def plot_BT(
    ...
    for i, ax in tqdm(enumerate(axes)):
        ...
        #ax.set_xlim(xlim[0] - fudge, xlim[1] + fudge - 10)
        # ax.set_xticks(np.arange(0, int(df.OR_up.max()) + 2, 1))
        #ax.set_xticks(np.arange(xlim[0], xlim[1] -7, 0.5))
...
def plot_QT(
    ...
    for i, ax in tqdm(enumerate(axes)):
        ...
        ax.set_yticks(temp.index)
        # ax.set_xlim(0 - fudge, df.OR_up.max() + fudge)
        ax.set_xlim(0, xlim[1] + fudge -1)
        # ax.set_xticks(np.arange(0, int(df.OR_up.max()) + 2, 1))
        ax.set_xticks(np.arange(xlim[0]+0.8, xlim[1]-0.3, 0.1))
